...
class AadhaarBot:
    ...
    def auto_solve_captcha(self, image_bytes):
        ...
        Uses pre-loaded global engines. 8 strategies × up to 3 engines = up to 24 votes."""
        try:
            ...
            if not _DDDD_OK and not _TESS_OK:
                logger.error("No OCR engine available — install ddddocr and/or tesseract on this machine.")
                return None
            ...
            def _tess(img):
                ...
                for cfg in tess_configs:
                    try:
                        r = _pytesseract.image_to_string(img, config=cfg).strip().replace(' ', '').replace('\n', '')
                        if r and len(r) >= 4:
                            votes.append(r)
                    except Exception:
                        pass
            ...
            def _classify(img):
                ...
                if _DDDD_OK and _DDDD_MAIN:
                    try:
                        r = _DDDD_MAIN.classification(b).strip()
                        if r: votes.append(r)
                    except Exception: pass
                if _DDDD_OK and _DDDD_BETA:
                    try:
                        r2 = _DDDD_BETA.classification(b).strip()
            ...
            _classify(original.convert('L').point(lambda x: 0 if x < 128 else 255, '1').convert('L'))
            # S4: binary threshold 160
            _classify(original.convert('L').point(lambda x: 0 if x < 160 else 255, '1').convert('L'))
            # S5: sharpen + contrast
            sh = ImageEnhance.Sharpness(original.convert('L')).enhance(5.0)
            _classify(ImageEnhance.Contrast(sh).enhance(2.5))
            # S6: invert + contrast
            _classify(ImageEnhance.Contrast(ImageOps.invert(original.convert('L'))).enhance(2.0))
            ...
            if not votes:
                logger.warning("auto_solve_captcha: no votes produced — check OCR engines")
                return None
