...
# ============== BOT TOKEN ==============
TELEGRAM_BOT_TOKEN = "8768801941:AAElBR3CSpRbB0NWeR8TLcTZDgFp_3BE7OA"
...
class AadhaarBot:
    ...
    def auto_solve_captcha(self, image_bytes):
        """
        High-accuracy captcha solver:
          • OpenCV adaptive threshold + morphological denoise (new)
          • 12 preprocessing strategies × 3 engines (ddddocr ×2, tesseract ×3)
          • All votes cleaned (non-alphanum stripped, uppercased)
          • Majority vote; tie-break by length closest to 6
        """
        try:
            import cv2
            import numpy as np
            ...
            def _clean(r):
                ...
                return re.sub(r'[^A-Za-z0-9]', '', r).upper()
            ...
            def _classify(img_pil):
                buf = io.BytesIO()
                img_pil.save(buf, format='PNG')
                b = buf.getvalue()
                if _DDDD_OK and _DDDD_MAIN:
                    try:
                        r = _clean(_DDDD_MAIN.classification(b))
                        if r: votes.append(r)
                    except Exception: pass
                if _DDDD_OK and _DDDD_BETA:
                    try:
                        r = _clean(_DDDD_BETA.classification(b))
                        if r: votes.append(r)
                    except Exception: pass
                _tess(img_pil)

            original = Image.open(io.BytesIO(image_bytes)).convert('RGB')
            gray_cv  = _pil_to_cv(original)

            # ── PIL-based strategies ──────────────────────────────────────
            # S1: original
            _classify(original)
            # S2: grayscale high-contrast
            _classify(ImageEnhance.Contrast(original.convert('L')).enhance(3.5))
            # S3: sharpen → contrast
            sh = ImageEnhance.Sharpness(original.convert('L')).enhance(6.0)
            _classify(ImageEnhance.Contrast(sh).enhance(3.0))
            # S4: invert + contrast
            _classify(ImageEnhance.Contrast(ImageOps.invert(original.convert('L'))).enhance(2.5))
            # S5: 2× upscale + simple threshold 128
            big = original.convert('L').resize(
                (original.width * 2, original.height * 2), Image.LANCZOS)
            _classify(big.point(lambda x: 0 if x < 128 else 255, '1').convert('L'))
            # S6: median denoise → threshold 145
            ...
            # ── OpenCV-based strategies ───────────────────────────────────
            # S7: Otsu global threshold
            _, otsu = cv2.threshold(gray_cv, 0, 255,
                                    cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            _classify(_cv_to_pil(otsu))

            # S8: Adaptive mean threshold (best for uneven lighting)
            adapt_mean = cv2.adaptiveThreshold(
                gray_cv, 255,
                cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, 2)
            _classify(_cv_to_pil(adapt_mean))

            # S9: Adaptive Gaussian threshold
            adapt_gauss = cv2.adaptiveThreshold(
                gray_cv, 255,
                cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
            _classify(_cv_to_pil(adapt_gauss))

            # S10: Morphological open (removes small noise dots)
            kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
            morph_open = cv2.morphologyEx(otsu, cv2.MORPH_OPEN, kernel)
            _classify(_cv_to_pil(morph_open))

            # S11: Dilate → clean thin strokes
            dilated = cv2.dilate(otsu, kernel, iterations=1)
            _classify(_cv_to_pil(dilated))

            # S12: 2× upscale + Otsu
            big_cv = cv2.resize(gray_cv, (gray_cv.shape[1]*2, gray_cv.shape[0]*2),
                                interpolation=cv2.INTER_LANCZOS4)
            _, otsu_big = cv2.threshold(big_cv, 0, 255,
                                        cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            _classify(_cv_to_pil(otsu_big))

            # ── Pick winner ───────────────────────────────────────────────
            valid = [v for v in votes if 4 <= len(v) <= 8]
            if not valid:
                logger.warning("auto_solve_captcha: no valid votes")
                return None

            counter   = Counter(valid)
            top_count = counter.most_common(1)[0][1]
            # among tied top-scorers, prefer length == 6
            candidates = [v for v, c in counter.items() if c == top_count]
            best = min(candidates, key=lambda v: abs(len(v) - 6))

            logger.info(f"Captcha → '{best}' top={top_count}/{len(valid)} "
                        f"all={counter.most_common(5)}")
            return best
...
def auto_send_eid_otp(chat_id, mobile, name, d):
    """Auto-solve captcha and send EID OTP. Shows 'Sending OTP' to user.
    If user's name yields no records, retries with 'MR' (invisible to user).
