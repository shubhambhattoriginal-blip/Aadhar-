...
PLANS = {
    ...
    '100': {'credits': 100, 'price': '$100', 'lifetime': False},   # Changed: 100 credits, no lifetime
}
...
def get_credits(user_id):
    ...
    if u is None:
        return 0
    # No lifetime concept now; but keep for backward compatibility
    if u.get('lifetime'):
        return float('inf')
...
def show_credits_info(chat_id):
    ...
    # No infinity symbol - display numeric value or "Lifetime" if set (though we may not use lifetime now)
    if cr == float('inf'):
...
def handle_callback(chat_id, callback_query_id, data):
    ...
    if data.startswith('buy_'):
        ...
        # Professional payment message with contact at bottom
        send_message(
            chat_id,
            f"<b>{BOT_NAME}</b>\n{DIVIDER}\n"
