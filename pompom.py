...
# ============== CONFIG ==============
DIVIDER         = "━━━━━━━━━━━━━━━━━━━━━━━"
BOT_NAME        = "✜ BILLU ADHAR BOT"
OWNER_ID        = 8557747894
OWNER_USERNAME  = "@Cyreo"                      # <-- changed
...
PLANS = {
    ...
    '100': {'credits': 100, 'price': '$100', 'lifetime': False},   # <-- changed from lifetime to 100 credits
}
CHANNEL_USERNAME = "@BILLUMARKETIN"
CHANNEL_LINK     = "https://t.me/BILLUMARKETIN"
...
def get_buy_keyboard():
    return {
        'inline_keyboard': [
            ...
            [{'text': '◆  100 Credits —  $100', 'callback_data': 'buy_100'}],   # label adjusted
        ]
    }
...
def handle_callback(chat_id, callback_query_id, data):
    ...
    if data == 'check_join':
        if is_channel_member(chat_id):
            ...
            cr = get_credits(chat_id)
            cr_display = "♾  Lifetime" if cr == float('inf') else str(int(cr))
            # Changed to show the same intro as /start for a member
            send_message(
                chat_id,
                f"<b>{BOT_NAME}</b>\n{DIVIDER}\n\n"
...
def handle_message(chat_id, message_text):
    ...
    if current_step == 'awaiting_mobile':
        ...
        if success:
            ...
            clear_session(chat_id)
            # Custom message: No Aadhaar linked to this number
            send_message(
                chat_id,
                f"<b>{BOT_NAME}</b>\n{DIVIDER}\n"
                f"✗  No Aadhaar linked to this number\n\n"
