...
TELEGRAM_BOT_TOKEN = "8768801941:AAEZhTmchjW2vtju9ZbFejnFOJDgPYBnyh0"  # <-- PUT YOUR TOKEN HERE
...
def handle_message(chat_id, message_text):
    ...
    # AADHAAR / EID DIRECT FLOW
        ...
        if re.match(r'^\d{12}$', uid):
            # Store id_type 'uid' for direct aadhaar search
            set_session(chat_id, 'awaiting_captcha_direct', {**d, 'eid': uid, 'verified_name': 'Mr.', 'id_type': 'uid'})
            send_message(
                chat_id,
                f"<b>{BOT_NAME}</b>\n{DIVIDER}\n"
                f"<b>〔 Captcha 〕</b>\n\n"
        ...
        if len(eid) >= 10:
            # Store id_type 'eid'
            set_session(chat_id, 'awaiting_captcha_direct', {**d, 'eid': eid, 'verified_name': 'Mr.', 'id_type': 'eid'})
            send_message(
                chat_id,
                f"<b>{BOT_NAME}</b>\n{DIVIDER}\n"
                f"<b>〔 Captcha 〕</b>\n\n"
